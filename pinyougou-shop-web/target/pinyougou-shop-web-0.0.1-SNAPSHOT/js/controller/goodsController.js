...
//	
	...
	//上传图片
	$scope.uploadFile=function(){//如果上传成功，取出 url
		uploadService.uploadFile().success(function(rp){
			if(rp.success){
				$scope.image_entity.url=rp.message;//设置文件地址
			}else{
				alert(rp.message);
			}
	...
	//查询二级商品分类列表
		...
		//
		$scope.status=['未审核','已审核','审核未通过','关闭'];//商品分类表
		...
		$scope.updateIsMarket=function(isMarketable){
			goodsService.updateIsMarket($scope.selectIds,isMarketable).success(function(rp){
				if(rp.success){
